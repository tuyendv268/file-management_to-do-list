...
public class NewTaskForm extends JDialog{
	...
	private void initialize() {
		...
		lblNewLabel = new JLabel("Form : yy-mm-dd");
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 14));
		lblNewLabel.setBounds(115, 140, 250, 30);
		getContentPane().add(lblNewLabel);
		
		lblFormYymmdd = new JLabel("Form : yy-mm-dd");
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(newTask == null) {
						...
						if(taskManager.saveTask(newTask)) {
							taskManager.insertLast(newTask);
						}
					}else {
						...
						taskManager.getTaskService().editTask(newTask, taskManager.getDataFile());
